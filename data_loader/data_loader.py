...
def Load_Dataset(dataset,
    ...
    SNRs = torch.tensor(SNRs, dtype=torch.float32)          # IMPROVEMENT B
...
def Dataset_Split(Signals,
                  Labels,
                  SNRs,                 # IMPROVEMENT B: tensor (N,) float32
                  snrs,
                  mods,
                  logger,
                  val_size=0.2,
    ...
    SNRs_train    = SNRs[train_idx]    # IMPROVEMENT B
    ...
    # IMPROVEMENT B: SNRs_train added to train tuple for per-sample loss weighting
    return (Signals_train, Labels_train, SNRs_train), \
