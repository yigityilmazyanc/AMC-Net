...
def snr_augment(sig, snr):
    ...
    # 4. Frequency masking: zero out 1–8 consecutive FFT bins (SpecAugment-style)
    spec    = torch.fft.rfft(sig, dim=-1)              # (B, 2, 65) complex
    n_freq  = spec.size(-1)                            # 65
    f_width = torch.randint(1, 9, (B,), device=device)
    f_start = torch.randint(0, n_freq - 8, (B,), device=device)
    for i in range(B):
        spec[i, :, f_start[i]:f_start[i] + f_width[i]] = 0
    sig = torch.fft.irfft(spec, n=128, dim=-1)         # (B, 2, 128)
