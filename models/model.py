...
class SEBlock(nn.Module):
    """Squeeze-and-Excitation channel attention (Hu et al., 2018), r=16."""
    ...
    def forward(self, x):
        # x: (B, C, 1, W)
        s = self.squeeze(x).view(x.size(0), -1)            # (B, C)
        e = self.excitation(s).view(x.size(0), -1, 1, 1)   # (B, C, 1, 1)
        return x * e
...
class AMC_Net(nn.Module):
    def __init__(self,
        ...
        self.SE  = SEBlock(self.conv_chan_list[-1])          # IMPROVEMENT C
    ...
    def forward(self, x):
        # x = x / x.norm(p=2, dim=-1, keepdim=True)  # removed: hurts amplitude-based classes
        x = x.unsqueeze(1)
        x = self.ACM(x)
        x = x / x.norm(p=2, dim=-1, keepdim=True)
        x = self.MSM(x)
        x = self.Conv_stem(x)
        x = self.SE(x)                                      # IMPROVEMENT C
